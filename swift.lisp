	...
	(with-output-to-string (s)
	  (format s "(~a)~@[-> ~a ~]"
		  ...
		  (let ((r (gethash 'return-values env)))
			...
			(funcall emit `(paren ,@r))
			(car r))))
	  (format s "in ~a" (funcall emit `(progn ,@body))))))))
...
(progn
  ...
  (defun emit-swift (&key code (str nil)  (level 0) (hook-defun nil))
    ...
    (flet ((emit (code &optional (dl 0))
	     ...
	     (emit-swift :code code :level (+ dl level) :hook-defun hook-defun)))
      ;(format t "~a~%" code)
      (if code
	  (if (listp code)
	      (progn
		(case (car code)
		  ...
		  #+nil (for (destructuring-bind ((start end iter) &rest body) (cdr code)
			 (format nil "for (~@[~a~];~@[~a~];~@[~a~]) ~a"
				 ...
				 (emit iter)
				 (emit `(progn ,@body)))))
		  (dotimes (destructuring-bind ((i n ; &optional (step 1)
						   ) &rest body) (cdr code)
			     (emit `(for (,i (..< 0 ,n))
					 ,@body))))
